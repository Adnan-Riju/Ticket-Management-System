...
namespace TicketManagementSystem.Controllers
{
    public class DashboardController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var userRoleId = vmuser.UserRoleId;

            // Set UserRoleId in ViewBag
            ViewBag.UserRoleId = userRoleId;

            // SQL query to get counts for each ticket status
            var sql = @"
        SELECT 
            SUM(CASE WHEN t.ManagementTicketStatus = 1 THEN 1 ELSE 0 END) AS ActiveCount,
            SUM(CASE WHEN t.ManagementTicketStatus = 2 THEN 1 ELSE 0 END) AS ProcessingCount,
            SUM(CASE WHEN t.ManagementTicketStatus = 3 THEN 1 ELSE 0 END) AS ClosedCount,
            SUM(CASE WHEN t.ManagementTicketStatus = 4 THEN 1 ELSE 0 END) AS DiscardCount,
            COUNT(*) AS TotalTicketCount
        FROM 
            Ticket t
        WHERE 
            (t.Assigned = @UserId OR t.IssuedBy = @UserId)
    ";

            using (var con = new SqlConnection(AppSetting.ConnectionString))
            {
                // Execute the SQL query and map the result to a view model
                ...
                // Prepare the summary list for different statuses
                var ticketSummaries = new List<TicketSummary>
        {
            ...
            new TicketSummary { StatusCount = result.TotalTicketCount, ActiveStatusString = "Total Tickets" }
        };
        ...
        public async Task<IActionResult> DiscardedTickets()
        {
            ...
            var userId = vmuser.Id;
            var tickets = await _context.Ticket
                .Where(t => t.ActiveStatus == 4 && t.IssuedBy == userId) // Assuming 1 is the code for Active status
                .Select(t => new TicketVm
                {
                    TicketId = t.Id,
                    TicketNo = t.TicketNo,
