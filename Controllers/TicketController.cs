...
namespace CompanyManagement.Controllers
{
    ...
    u.UserName AS IssuedByName FROM    Ticket t    INNER JOIN [User] u ON t.IssuedBy = u.Id    INNER JOIN Product ticket ON t.ProductId = ticket.Id     INNER JOIN ServiceType s ON t.ServiceTypeId = s.Id    LEFT JOIN TicketType tt ON t.TicketTypeId = tt.Id ";
        ...
        public IActionResult CreateTicket()
        {
            ...
            // Fetch products synchronously based on user products that are active
            var products = userProducts
                .Where(i => i.Product.ActiveStatus == 1) // Add condition for active status
                .Select(i => i.Product)
                .ToList();
        ...
        public async Task<IActionResult> GetTicketDetails(int id)
        {
            ...
            var ticket = await _context.Ticket
                ...
                .Select(t => new TicketVm
                {
                    ...
                    AssignedName= _context.User.Where(u => u.Id == t.Assigned).Select(u => u.UserTitle).FirstOrDefault(),
                    IssuedByName = _context.User.Where(u => u.Id == t.IssuedBy).Select(u => u.UserTitle).FirstOrDefault(),
                    CompleteByName = t.CompleteBy.HasValue ? _context.User.Where(u => u.Id == t.CompleteBy.Value).Select(u => u.UserTitle).FirstOrDefault() : null
                })
                .FirstOrDefaultAsync();        
        ...
        public async Task<IActionResult> Assign(int id)
        {
            ...
            var users = await _context.UserProduct
                ...
                .Select(up => new
                {
                    User = _context.User.FirstOrDefault(u => u.Id == up.UserId)
                })
                .Where(u => u.User != null && (u.User.UserRoleId == 5 || u.User.UserRoleId == 8)) // Add condition here
                .Select(up => new SelectListItem
                {
                    Value = up.User.Id.ToString(),
                    Text = up.User.UserName
                }).ToListAsync();
        ...
        public async Task<JsonResult> Assign(Ticket Ticket)
        {
            ...
            return Json(new 
            {
                ...
                redirectUrl = Url.Action("GetAllTickets", "Ticket") 
            });
        }
...
";
        ...
        public async Task<IActionResult> ProcessingTickets()
        {
            ...
            var tickets = await _context.Ticket
                ...
                .Select(t => new TicketVm
                {
                    ...
                    ActiveStatus = t.ActiveStatus,
                    IssuedByName = _context.User.Where(u => u.Id == t.IssuedBy).Select(u => u.UserTitle).FirstOrDefault(),
                    AssignedName = t.Assigned.HasValue ? _context.User.Where(u => u.Id == t.Assigned.Value).Select(u => u.UserTitle).FirstOrDefault() : "Not Assigned",
                    CompleteByName = t.CompleteBy.HasValue ? _context.User.Where(u => u.Id == t.CompleteBy.Value).Select(u => u.UserTitle).FirstOrDefault() : null
                })
                .ToListAsync();
        ...
        public IActionResult Communicate(int id)
        {
            ...
            if (ticket == null)
            {
                return NotFound();
                   ...
                   var previousMessages = _context.TicketChat.Where(tc => tc.TicketId == id).OrderBy(tc => tc.CreatedDate)
                    .Select(tc => new TicketChatMessageViewModel
                    {
                            Id = tc.Id,
                            TicketId = tc.TicketId,
                            UserId = tc.UserId,
                            Product = _context.Product.Where(p => p.Id == tc.Id).Select(p => p.ProductName).FirstOrDefault() ?? "Unknown Product",

                            ChatMessage = tc.ChatMessage,CreatedDate = tc.CreatedDate,TicketSubject = _context.Ticket.
                            Where(t => t.Id == tc.Id).Select(t => t.TicketSubject).FirstOrDefault() ?? "Unknown Subject",

                            TicketDetails = _context.Ticket.Where(t => t.Id == tc.Id).Select(t => t.TicketDetails).FirstOrDefault() ?? "Unknown Details",
                            FileUploadPath = tc.FileUploadPath,
                            UserTitle= _context.User.Where(u => u.Id == tc.UserId).Select(u => u.UserTitle).FirstOrDefault() ?? "Unknown User",
                        Username = _context.User.Where(u => u.Id == tc.UserId).Select(u => u.UserName).FirstOrDefault() ?? "Unknown User",
                    
                    }).ToList();


        var assignedUserName = _context.User
                .Where(u => u.Id == ticket.Assigned) // Assuming 'Assigned' is the correct field
                .Select(u => u.UserTitle)
            ...
            var viewModel = new TicketChatViewModel
            {
                TicketId = id,
                UserId =(int) userId,
                Username = _context.User.FirstOrDefault(u => u.Id == userId)?.UserName,
                UserTitle = _context.User.FirstOrDefault(u => u.Id == userId)?.UserTitle,
                PreviousMessages = previousMessages,
                CreatedDate = DateTime.Now,
                Product = _context.Product.FirstOrDefault(p => p.Id == ticket.ProductId)?.ProductName ?? "Unknown Product",
                Status = ticket.ActiveStatus.ToString(),
                TicketDetails=ticket.TicketDetails,
                TicketSubject=ticket.TicketSubject,
                AssignedBy = assignedUserName // Include assigned user name in the view model
            };
        ...
        public async Task<IActionResult> Communicate(TicketChatViewModel viewModel)
        {
            ...
            var ticketChat = new TicketChat
            {
                ...
                UserId = (int)userId,
                ChatMessage = viewModel.ChatMessage,
                CreatedDate = DateTime.Now,
                FileUploadPath = viewModel.FileUpload != null ? await SaveFileAsync(viewModel.FileUpload) : null
            };
        ...
        {
            var filePath = Path.Combine("uploads", file.FileName); // Adjust the path as needed
        ...
        public IActionResult AssignedTickets()
        {
            ...
            // Get the user titles for the assigned and issued users
            var assignedUserTitle = _context.User
                .Where(u => u.Id == userId) // Change this to match the correct ID
                .Select(u => u.UserTitle)
                .FirstOrDefault() ?? "Unknown User";

            // You might want to get the issued user title based on the ticket
            var issuedUserTitle = assignedTickets.FirstOrDefault()?.IssuedBy != null
                ? _context.User
                    .Where(u => u.Id == assignedTickets.First().IssuedBy) // Assuming IssuedBy is a user ID
                    .Select(u => u.UserTitle)
                    .FirstOrDefault() ?? "Unknown User"
                : "Unknown User";
            ...
            var viewModel = new AssignedTicketsViewModel
            {
                ...
                AssignedUser = assignedUserTitle,
                IssuedUser = issuedUserTitle,
                ActiveUsers = _context.User.ToList() // Fetch all active users
            };
